...
MAP22(frame,bdryPts,ctrlPts,mu,alpha0,sizes,Sigma_small,Sigma_big,Sigma_0,Sigma_1,dt,nofSteps,par_obs,delta,nofIt, prior)
...
% Constructing the covariance kernel matrix
  if (~isempty(Sigma_big)) 
    ...
    C = Cov_block(ctrlPts(1:sizes(2),:),ctrlPts(sizes(2)+1:end,:),Sigma_small^2,Sigma_big^2);
  else 
    % construct a homogeneous covariance matrix
    C = K_matrix(ctrlPts,ctrlPts,2*Sigma_0^2 - 2*Sigma_1^2 + Sigma_small^2 );
  end
    C = blkdiag(C,C);
...
Sigma_inv = blkdiag(K,K)*inv(C)*blkdiag(K,K);
...
for it = 1:nofIt
  ...
  E(it) = - L + prior*alpha(:)'*Sigma_inv*alpha(:)/2; 
  % disp(E(it))
  ...
  step = 1/(size(m,1)-sizes(1,1));
  [m2, nu2] = arcLength(m(sizes(1,1)+1:end,:,it),step); 
  sizes(1,1) = size(m1,1);
  m(:,:,it) = [m1;m2];
...
%    G'*g(:)
%    Sigma_inv*(alpha(:) - mu(:))
  gg = reshape((G'*g(:) + prior*Sigma_inv*(alpha(:) - mu(:))),nofCtrlPts,2);
