...
function res = trackVideoJacobian1(frames,m_average,x_average,alpha_average,sizes,movieName,frameName)
...
for i=1:1:size(m_average,3)-1
    ...
    Z = griddata(pts_new(:,1),pts_new(:,2),J,X,Y,'v4');
    ...
    colormap(jet)
    hold on ;
    plot(pts_new(:,1),pts_new(:,2),'ko')
    plot(x_new(:,1),x_new(:,2),'bo','Linewidth',2)
