...
x_i_new = x_i;
alpha_new = alpha;
[dx dalpha] = updateODE(m,x_i_new,alpha,Sigma_0);
dalpha
[dx dalpha] = updateODE1(x_i_new,alpha,alpha,Sigma_0,size(alpha,1),2);
dalpha
pause()
...
for step = 1:nofSteps
    ...
    dist = bsxfun(@minus,x_i(:,1),x_i_new(:,1).').^2 + ...
           bsxfun(@minus,x_i(:,2),x_i_new(:,2).').^2;
    ...
    dist1 = bsxfun(@minus,x_i(:,1),x_i_new(:,1)');
    dist2 = bsxfun(@minus,x_i(:,2),x_i_new(:,2)');
    ...
    alpha_new(:,1) = alpha(:,1) - dt*(squeeze(Dv(1,1,:)).*alpha(:,1)+squeeze(Dv(1,2,:)).*alpha(:,2));
    alpha_new(:,2) = alpha(:,2) - dt*(squeeze(Dv(2,1,:)).*alpha(:,1)+squeeze(Dv(2,2,:)).*alpha(:,2));
